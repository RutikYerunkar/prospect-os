...
class TestReconcileEndpoint:
    async def test_reconcile_found_settles_succeeded(self, client, session_factory, monkeypatch):
        proposal, _ = await _live_approved_proposal(client, session_factory, monkeypatch)
        now = utcnow()
        ...
        runtime, _ = make_runtime(
            handler=_gmail_handler(
                ...
                get_body_by_id={"m1": {"payload": {"headers": [{"name": "Message-ID", "value": "<abc@example.com>"}]}}},
            )
        )
    ...
    async def test_reconcile_not_found_stays_uncertain_never_failed(self, client, session_factory, monkeypatch):
        ...
        runtime, _ = make_runtime(
            handler=_gmail_handler(
                list_body={"messages": [{"id": "m1"}]},
                get_body_by_id={"m1": {"payload": {"headers": [{"name": "Message-ID", "value": "<nomatch@example.com>"}]}}},
            )
        )
        _override_runtime(runtime)
        try:
    ...
    async def test_window_expired_abandons_with_zero_egress(self, client, session_factory, monkeypatch):
        ...
        execution_id = await _insert_execution(
            session_factory, proposal=proposal, status="UNCERTAIN", claimed_at=long_ago, dispatched_at=long_ago
        )
    ...
    async def test_reconcile_requires_operator(self, client, session_factory, monkeypatch):
        ...
        now = utcnow()
        execution_id = await _insert_execution(
            session_factory, proposal=proposal, status="UNCERTAIN", claimed_at=now, dispatched_at=now
        )
